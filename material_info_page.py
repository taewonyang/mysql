...
class Material_Info :
    ...
    def layout(self):
        ...
        global name_txt, namecode_eng_cmb, namecode_kor_lb, kind_cmb, hscode_txt, listbox

        name_lb = Label(self.window, text='원자재 규격')
        name_lb.place(x=10, y=60)
        name_txt = Entry(self.window, width=20)
        name_txt.place(x=120, y=60)
        ...
        namecode_kor_lb = Label(self.window, width=20, anchor='w')
        namecode_kor_lb.place(x=120, y=120)
        kind_lb = Label(self.window, text='원자재 종류')
        kind_lb.place(x=360, y=60)
        kind_cmb = ttk.Combobox(self.window, height=2, width=9, values=['원석','기타'], state='readonly')
        kind_cmb.place(x=460, y=60)
        hscode_lb = Label(self.window, text='세번(HS CODE)')
        hscode_lb.place(x=360, y= 90)
        hscode_txt = Entry(self.window, width=12)
        hscode_txt.place(x=460, y=90)
        ...
        update_btn = Button(self.window, text='Data\nUpdate')
        update_btn.place(x=520, y=200)
        delete_btn = Button(self.window, text='Data\nDelete')
        delete_btn.place(x=520, y=250)

        def changeText(event) :
            if namecode_eng_cmb.get() == "SCD" :
                namecode_kor_lb.configure(text='단결정 다이아몬드')
            elif namecode_eng_cmb.get() == "PCD" :
                namecode_kor_lb.configure(text='다결정 다이아몬드')
            elif namecode_eng_cmb.get() == "SHANK" :
                namecode_kor_lb.configure(text='샹크')
    ...
    def create_tree_widget(self):
        ...
        tree.column('namecode_eng_column', width=60, anchor=CENTER)
    ...
    def regist(self):
        if name_txt.get() == "":
            ...
            msgbox.showerror("입력오류!", "품명(영문)을 입력해주세요")
        elif kind_cmb.get() == "":
            msgbox.showerror("입력오류!", "원자재 종류를 선택해주세요")
        elif hscode_txt.get() == "":
            ...
            # 중복여부 체크
            data = (name_txt.get(), namecode_eng_cmb.get(), namecode_kor_lb.cget('text'), kind_cmb.get(),hscode_txt.get())
            overlap_check = []
            if rs != []:  # DB에 데이터가 있다면
                for row in rs:
                    if row[1:] == tuple(data):
            ...
            # (데이터O / 중복X) or (데이터X)
                ...
                cur.execute(insert_sql, (name_txt.get(), namecode_eng_cmb.get(), namecode_kor_lb.cget('text'), kind_cmb.get(),hscode_txt.get()))
    ...
    def cancel(self):
        ...
        namecode_kor_lb.configure(text='')
        kind_cmb.set("")
        hscode_txt.delete(0, END)
